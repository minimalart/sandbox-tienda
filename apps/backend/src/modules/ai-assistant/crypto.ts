...
/**
 * Cifrado en reposo de las credenciales de los servidores MCP externos
 * (tokens estáticos y tokens OAuth). AES-256-GCM con clave derivada de
 * `JWT_SECRET` vía scrypt. Formato del blob: `v1:<iv b64>:<tag b64>:<ct b64>`.
 *
...
function deriveKey(): Buffer {
  ...
  if (!cache || cache.secret !== secret) {
    cache = { secret, key: scryptSync(secret, 'ai-mcp-server-v1', 32) };
  }
  return cache.key;
}
 ...
 * y marcar el servidor en error.
 */
export function decryptSecret(blob: string): string {
  const parts = blob.split(':');
  const [version, ivB64, tagB64, ctB64] = parts;
  if (parts.length !== 4 || version !== VERSION || !ivB64 || !tagB64 || !ctB64) {
    throw new Error('Secreto MCP con formato inválido.');
  }
