...
// ─── S3 image upload field for Puck sidebar ──────────────────────────────────

/**
 * Custom Puck field that renders a URL text input + an upload-to-S3 button in
 * the Puck component sidebar. The user can paste a URL directly or pick a file;
 * either way, `onChange` receives the final public URL string.
 */
function S3ImageFieldRender({
  value,
  onChange,
  label = 'URL de imagen',
}: {
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      // Silent — upload errors show in the network tab; the user can paste manually.
    } finally {
      setUploading(false);
    }
  };
...
const s3ImageField = (fieldLabel: string) => ({
