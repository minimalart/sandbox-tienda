...
/**
 * El inverso de `ringToPolygonPoints`: los vértices guardados → un anillo
 * GeoJSON cerrado (`[lng, lat][]`).
 *
 * Existe porque las zonas del store locator se persisten como GeoJSON estándar
 * mientras que el picker del mapa trabaja con `{x, y}` strings. Sin este par,
 * cada consumidor se escribía su propia conversión — y el orden lng/lat es
 * exactamente la clase de detalle que se invierte una vez y no se nota hasta
 * que un filtro deja de matchear.
 */
export const polygonPointsToRing = (points: PolygonPoint[]): [number, number][] => {
  const ring: [number, number][] = points
    .map((p): [number, number] => [Number(p.x), Number(p.y)])
