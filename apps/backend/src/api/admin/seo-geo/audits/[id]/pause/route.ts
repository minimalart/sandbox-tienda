...
/**
 * POST /admin/seo-geo/audits/:id/pause — pausa una auditoría. Si está en curso,
 * el crawl corta en el próximo lote. Se reanuda re-corriendo (/resume).
 */
export async function POST(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  if (!['queued', 'running'].includes(audit.status)) {
    res.status(409).json({ type: 'invalid_state', message: `No se puede pausar una auditoría ${audit.status}` });
    return;
  }
