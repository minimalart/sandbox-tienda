...
export default async function TechRetailHome({
  ...
  return (
    <main className="tech-retail-home bg-white">
      ...
      {(!section || section === '1') && (<TrHero config={tr?.hero} />)}

      {/* 3. Categorías destacadas */}
      {(!section || section === '2') && (<TechCategoryGrid config={tr?.categories} />)}

      {/* 4. Marcas destacadas (bloque obligatorio) */}
      {(!section || section === '3') && (<BrandCarousel config={tr?.brands} />)}

      {/* 5. Financiación / promociones */}
      {(!section || section === '4') && (<FinancingBanner config={tr?.financing} />)}

      {/* 6. Productos destacados (catálogo dinámico) */}
      {(!section || section === '5') && (<Suspense fallback={null}>
        <TrFeaturedProducts countryCode={countryCode} />
      </Suspense>)}

      {/* 7. Gaming (opcional) */}
      {(!section || section === '6') && (<TrPromoBanner config={tr?.gaming} ctaVariant="accent" />)}

      {/* 8. Home Office (opcional) */}
      {(!section || section === '7') && (<TrPromoBanner config={tr?.homeOffice} ctaVariant="primary" />)}

      {/* 9. Beneficios de compra */}
      {(!section || section === '8') && (<TrBenefits config={tr?.benefits} />)}

      {/* 10. Newsletter / novedades */}
      {(!section || section === '9') && (<TrNewsletter config={tr?.newsletter} />)}
    </main>
  );
}
