...
export default function GiftCardLanding({ token }: { token: string }) {
  ...
  if (state === 'loading') return <main className='flex min-h-[60vh] items-center justify-center'><Loader2 className='h-8 w-8 animate-spin text-emerald-700' /></main>
  if (state === 'claimed' && !card) return <main className='mx-auto flex min-h-[60vh] max-w-lg flex-col items-center justify-center px-4 text-center'><CheckCircle2 className='h-12 w-12 text-emerald-600' /><h1 className='mt-4 text-2xl font-bold'>Este saldo ya está acreditado en tu cuenta</h1><a className='mt-6 rounded-xl bg-[var(--primary-color)] px-6 py-3 font-bold text-white' href={`/${countryCode}/account/gift-cards`}>Ver mi billetera</a></main>
  ...
  return (
    <main className='mx-auto max-w-3xl px-4 py-10 sm:py-16'>
      <div className='overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-xl'>
        <div className='relative aspect-[1.9/1] bg-emerald-900'>{/* eslint-disable-next-line @next/next/no-img-element */}<img alt='' className='absolute inset-0 h-full w-full object-cover' src={card.design.desktop_image_url ?? '/images/gift-card-default.svg'} /><div className='absolute inset-0 flex flex-col items-center justify-center p-8 text-center' style={{ color: card.design.text_color ?? '#fff' }}><p className='text-sm font-bold uppercase tracking-[.25em]'>Un regalo para vos</p><p className='mt-4 text-4xl font-black'>{formatMoney(card.value, card.currency_code)}</p><p className='mt-2 font-mono text-sm'>{card.masked_code}</p></div></div>
        <div className='p-6 text-center sm:p-10'>
          ...
          {state === 'claiming' && <p className='mt-8 inline-flex items-center gap-2 font-semibold text-emerald-700'><Loader2 className='h-5 w-5 animate-spin' /> Acreditando tu saldo…</p>}
          {state === 'claimed' && <div className='mt-8'><p className='inline-flex items-center gap-2 font-bold text-emerald-700'><CheckCircle2 className='h-6 w-6' /> Saldo acreditado en tu cuenta</p><div className='mt-6 flex flex-col justify-center gap-3 sm:flex-row'><a className='inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--primary-color)] px-6 py-3 font-bold text-white' href={`/${countryCode}/store`}><ShoppingBag className='h-4 w-4' /> Comprar ahora</a><a className='rounded-xl border border-slate-200 px-6 py-3 font-bold' href={`/${countryCode}/account/gift-cards`}>Ver mi billetera</a></div></div>}
        </div>
      </div>
    </main>
  )
