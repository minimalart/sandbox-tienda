 ...
 */
export class Migration20260630210000 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `do $$ begin if not exists (select 1 from pg_constraint where conname = 'shop_by_look_product_look_id_foreign') then alter table "shop_by_look_product" add constraint "shop_by_look_product_look_id_foreign" foreign key ("look_id") references "shop_by_look" ("id") on update cascade on delete cascade; end if; end $$;`
    );
  }
