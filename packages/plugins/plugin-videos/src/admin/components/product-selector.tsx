...
/**
 * Vendored copy of the host's `ProductSelector`. In the base extension it was
 * imported from `apps/backend/src/admin/components/blog/product-selector`
 * (owned by the blog extension). The plugin can not reach that host path, so
 * we ship a self-contained version here.
 *
 * Differences with the original:
 * - Uses the `videos` translation namespace with `defaultValue` fallbacks so
 *   the copy works even without the blog namespace registered.
 * - Talks to `/admin/products` via plain `fetch` instead of the host's `sdk`
 *   admin client (same-origin, cookie-based session).
 */
...
async function listProducts(params: Record<string, string | string[] | number>): Promise<AdminProductLite[]> {
  ...
  if (!res.ok) return [];
  const data = (await res.json()) as { products?: AdminProductLite[] };
    ...
    <div className="flex flex-col gap-3">
      ...
      {value.length === 0 ? (
        <Text size="small" className="text-ui-fg-muted">
          {t('PRODUCTS_EMPTY', { defaultValue: 'No products linked yet.' })}
        </Text>
      ) : (
        <DndContext
          sensors={sensors}
