 ...
 */
export const rawValue = (prop: unknown): unknown =>
  prop && typeof prop === 'object' && 'value' in (prop as Record<string, unknown>)
...
export function lowConfidenceOf(
  prop: unknown,
  opts: { requireReview: boolean; threshold: number }
): number | null {
  if (!opts.requireReview) return null;
  if (!prop || typeof prop !== 'object' || !('confidence' in prop)) return null;
  const c = (prop as { confidence: unknown }).confidence;
...
export type AcceptAllPlan = {
  ...
  deferred: Array<{ field: string; confidence: number }>;
};
 ...
 */
export function planAcceptAll(opts: {
  proposed: FieldMap;
  accepted: FieldMap;
  rejected: FieldMap;
  ...
  const plan: AcceptAllPlan = { accept: {}, unreject: [], deferred: [] };

  for (const [field, value] of Object.entries(opts.proposed)) {
    if (field in opts.accepted || field in opts.rejected) continue;
    ...
    plan.accept[field] = rawValue(value);
