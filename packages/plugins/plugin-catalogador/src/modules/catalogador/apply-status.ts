 ...
 */
export const PENDING_REVIEW_STATUSES = ['proposed', 'generating', 'pending'];
 ...
 */
export const RETRYABLE_PRODUCT_STATUSES = ['accepted', 'apply_failed'];
 ...
 */
export const UNFINISHED_PRODUCT_STATUSES = [
 ...
 */
export function resolveApplyFinalStatus({
  applied,
  failed,
  unfinished,
}: ApplyOutcome): ApplyFinalStatus {
  if (failed > 0) return applied === 0 ? 'error' : 'partially_applied';
