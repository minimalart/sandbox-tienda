...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  Object.assign(filters, await siteFilter(req.scope, await siteFromRequest(req), LOYALTY_EARN_RULE_SITE_SCOPE));
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const service = req.scope.resolve<LoyaltyModuleService>(LOYALTY_MODULE);
  const created = await service.createEarnRules(req.validatedBody as Record<string, unknown>);
