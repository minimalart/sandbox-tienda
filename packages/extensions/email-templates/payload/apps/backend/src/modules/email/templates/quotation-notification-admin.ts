...
/**
 * Plantilla para notificación de cotización (draft order creada desde carrito).
 * Mensaje: "Se generó una nueva cotización" con resumen de productos y totales.
 * Misma estructura visual que order-notification-admin pero con textos de cotización.
 */
export const quotationNotificationAdminTemplate: EmailTemplateFunction<QuotationNotificationAdminData> = (
  data
): EmailTemplateResult => {
  ...
  const subject = 'Se generó una nueva cotización';
  const titleMain = 'Se generó una nueva cotización';
