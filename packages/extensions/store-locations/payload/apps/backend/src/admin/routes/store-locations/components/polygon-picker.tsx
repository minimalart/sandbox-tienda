...
export const PolygonPicker = ({
  ...
  const attachEditablePolygon = (map: GMap, maps: GMapsNS, ring: LatLngLike[]) => {
    ...
    pathListenersRef.current.push(
      polygon.addListener('rightclick', (...args: unknown[]) => {
        ...
        if (p.getLength() <= 3) {
          toast.error(t('COVERAGE_DRAW_NEED_THREE'));
          return;
        }
  ...
  const handleFile = async (file: File) => {
    ...
    if (result.areas.length > 1) {
      toast.warning(t('COVERAGE_GEOJSON_MULTI', { n: result.areas.length }));
    }
    ...
    <div className="flex flex-col gap-2">
      ...
      {hasPolygon && !isDrawing && (
        <Text size="small" className="text-ui-tag-green-text">
          {t('COVERAGE_PREVIEW_POINTS', { n: pointCount })}
        </Text>
      )}

      {isDrawing && (
        <Text size="small" className="text-ui-fg-subtle">
          {t('COVERAGE_PREVIEW_POINTS', { n: drawCount })}
        </Text>
      )}
      ...
      {apiKey && !hasMapError ? (
        ...
        <Text size="small" className="text-ui-fg-subtle">
          {apiKey ? t('MAPS_KEY_ERROR') : t('MAPS_NO_KEY_HINT')}
        </Text>
      )}
      ...
      <Text size="small" className="text-ui-fg-subtle">
        {editable
          ? isDrawing
            ? t('COVERAGE_DRAW_HINT')
            : hasPolygon
              ? t('COVERAGE_EDIT_HINT')
              : t('COVERAGE_MAP_HINT')
          : t('COVERAGE_MAP_HINT')}
      </Text>
    </div>
