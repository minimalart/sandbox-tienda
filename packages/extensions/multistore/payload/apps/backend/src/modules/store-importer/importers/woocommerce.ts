...
export type VariationPrice = { price: number; listPrice: number; externalVariantId?: string };
...
export async function fetchWooVariationPrices(
  ...
  for (const variation of variations ?? []) {
    ...
    try {
      ...
      map.set(slug, { price, listPrice, externalVariantId: String(variation.id) });
    } catch {
    ...
    if (built.length > 0) {
      ...
      if (variationPrices.size > 0) {
        for (const variant of built) {
          ...
          if (match) {
            variant.externalVariantId = match.externalVariantId;
